...
def serialize_gym_space(space: gym.Space, serialization_format=SerializationFormat.STRUCTURED) -> Space:
    if isinstance(space, (gym.spaces.Discrete, gym.spaces.Discrete)):
        return Space(discrete=Discrete(n=space.n, start=space.start))
...
def deserialize_space(pb_space: Space) -> gym.Space:
